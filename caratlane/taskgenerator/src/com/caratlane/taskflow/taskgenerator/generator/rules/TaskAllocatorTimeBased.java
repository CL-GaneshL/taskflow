 ...
 * @author wdmtraining
 */
class TaskAllocatorTimeBased implements TaskAllocator {
     ...
     */
    public TaskAllocatorTimeBased(final LocalDateTime allocation_from) {
    ...
    private Integer allocateTaskAllocation(
        ...
        final EmployeeSkillSuitability employeeSkillSuitability
                = (new EmployeeSkillSuitabilityFactory(TIME_BASED)).getInstance();

        final EmployeeData employeeData
                = employeeSkillSuitability.findMostSuitableEmployee(
        ...
        LocalDateTime av_start_date = employeeAvailability
                .getNextAvailability(employeeData, this.allocation_from);
        ...
        if (correctedDuration <= available) {
            ...
            // ==============================================
            // WD : TaskFlow is no longer Time base
            // this is commented out so it can compile
            // as the Nb_product_planned is now an Integer
            // ==============================================
//            double nb_products_planned = this.round(
//                    (double) duration / skill_duration, TWO_DECIMALS);
            int nb_products_planned = 0; // !!!!!!!!!!!!!!!!!!!!!!!!
